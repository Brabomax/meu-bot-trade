...
# ════════════════════════════════════════════════════════════════════════
# BANCO DE DADOS SQLITE (COM WAL PARA MULTI-USUÁRIO)
# ════════════════════════════════════════════════════════════════════════
...
def init_database():
    ...
    cursor = conn.cursor()
    cursor.execute('PRAGMA journal_mode=WAL;')  # 🚀 MELHORIA: Permite leituras/escritas simultâneas sem travar
    cursor.execute('''
...
# 🔒 NOVA FUNÇÃO: Protege contra múltiplas sessões da mesma conta IQ Option
def verificar_email_em_uso(email, exclude_sid=None):
    """Verifica se o e-mail já está rodando em OUTRA sessão"""
    conn = sqlite3.connect(DB_PATH, timeout=10, check_same_thread=False)
    cursor = conn.cursor()
    cursor.execute('SELECT bot_id FROM active_bots WHERE email = ?', (email,))
    result = cursor.fetchone()
    conn.close()
    
    if result:
        running_sid = result[0]
        # Se for o mesmo usuário apenas reiniciando, permite
        if exclude_sid and running_sid == exclude_sid:
            return False 
        # Se estiver registrado nos processos ativos do Python, está em uso
        if running_sid in processos:
            return True 
    return False
...
# ═══════════════════════════════════════════════════════════════════════
# PARSER DE SINAIS DO TELEGRAM (COM NORMALIZAÇÃO MELHORADA)
# ═══════════════════════════════════════════════════════════════════════
def parsear_sinal_telegram(texto):
    try:
        ...
        if m:
            par = m.group(1).strip()
            # 🚀 MELHORIA: Normaliza o nome do par para evitar conflitos (ex: EUR/USD -> EURUSD)
            par = par.replace('/', '').replace(' ', '').upper()
    ...
    try:
        ...
        while True:
            try:
                ...
                # ═══════════════════════════════════════════════════════════
                # LIMPEZA DE MEMÓRIA E BANCO DE DADOS A CADA 1 HORA
                # ═══════════════════════════════════════════════════════════
                if now.minute == 0 and now.second == 0:
                    ...
                    # 🗄️ LIMPEZA DO BANCO DE DADOS (Bots mortos)
                    try:
                        conn = sqlite3.connect(DB_PATH, timeout=5)
                        cursor = conn.cursor()
                        # Deleta bots sem heartbeat há mais de 10 minutos
                        cursor.execute("DELETE FROM active_bots WHERE julianday('now') - julianday(last_heartbeat) > (10.0/1440.0)")
                        conn.commit()
                        conn.close()
                    except Exception as e:
                        atualizar_log(f"⚠️ Erro na limpeza do DB: {str(e)}\n")
...
logs_web = {}  # Será substituído por manager.dict() no __main__
processos = {}
tg_sessions_storage = {}
manager = None
...
# ════════════════════════════════════════════════════════════════════════
# HTML COMPLETO (Mantido idêntico ao original para não quebrar o frontend)
# ════════════════════════════════════════════════════════════════════════
HTML_SISTEMA = """
<!DOCTYPE html>
<html>
...
# 🚀 ROTA /ligar ATUALIZADA PARA MULTI-USUÁRIO
...
def ligar():
    ...
    if not email:
        return jsonify({"erro": "❌ E-mail é obrigatório."})

    # 🔒 PROTEÇÃO MULTI-USUÁRIO: Impede 2 sessões diferentes com o mesmo e-mail
    if verificar_email_em_uso(email, exclude_sid=sid):
        return jsonify({"erro": f"⚠️ A conta {email} já está rodando em outro dispositivo/sessão. Pare-a primeiro."})

    # Se este SID já estiver rodando, encerra o antigo para iniciar o novo (reinício limpo)
    if sid in processos:
        try:
            processos[sid].terminate()
            processos[sid].join(timeout=2)
        except:
            pass
        del processos[sid]
    ...
    # Salva no DB com o novo SID
    salvar_bot_db(sid, email, d)

    # Inicia o processo isolado para este usuário
    p = multiprocessing.Process(target=loop_robo, args=(sid, d, logs_web))
    p.daemon = False
    p.start()
    processos[sid] = p

    return jsonify({"s": "ok", "msg": f"✅ Bot iniciado com sucesso para {email}"})

# 🚀 ROTA /parar ATUALIZADA PARA LIMPEZA COMPLETA
...
def parar():
    ...
    # Limpa o log da memória
    if sid in logs_web:
        del logs_web[sid]
        
    # Libera o e-mail no banco de dados
    remover_bot_db(sid)
    
    return jsonify({"ok": True, "msg": "Bot parado e sessão liberada."})
...
if __name__ == '__main__':
    global logs_web, manager
    print("SHIELD V37 - IQ OPTION + TELEGRAM (MULTI-USUÁRIO)")
